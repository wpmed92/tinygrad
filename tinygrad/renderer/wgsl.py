...
wgsl_matcher = PatternMatcher([
  ...
  (UPat(Ops.LOAD, name="l", src=(UPat.var('b'), UPat.var('c'), UPat())),
   lambda l,b,c: packed_load(l,b,l.dtype,c.cast(dtypes.uint32)) if l.dtype.itemsize < 4 and l.dtype != dtypes.half else None),
  (UPat.store(UPat.var("bidx"), UPat.var("var"), allow_any_len=True), lambda bidx,var: packed_store(bidx,var) if var.dtype.itemsize < 4 and var.dtype != dtypes.half else None),
...
class WGSLRenderer(CStyleLanguage):
  ...
  string_rewrite = PatternMatcher([
    ...
    (UPat(Ops.BITCAST, name="x", dtype=(dtypes.uchar,dtypes.char)), lambda ctx,x: f"bitcast<{ctx.type_map[x.dtype]}>({ctx[x.src[0]]}&0xFF)"),
    (UPat(Ops.BITCAST, name="x", dtype=(dtypes.short,dtypes.ushort)), lambda ctx,x: f"bitcast<{ctx.type_map[x.dtype]}>({ctx[x.src[0]]}&0xFFFF)"),
    ...
    (UPat.store(UPat.var('b'), UPat.var("v"), allow_any_len=True),lambda ctx,b,v:\
     ...
     f"atomicAnd(&{ctx[b]},{ctx[v.src[0].src[1]]});\n  atomicAdd(&{ctx[b]},{ctx[v.src[1]]});" if b.src[0].dtype.itemsize < 4 and b.src[0].dtype.base != dtypes.half \
      else f"{ctx[b]} = {ctx[v]};"),
  ...
  def render_load(self, x:str, dt:DType) -> str: return f"atomicLoad(&{x})" if dt.itemsize < 4 and dt.base != dtypes.half else x
