...
class WebGPUProgram:
  ...
  def __call__(self, *bufs, global_size=(1,1,1), local_size=(1,1,1), vals=(), wait=False):
    ...
    for i in range(1, len(raw_bufs)):
      if raw_bufs[i] == raw_bufs[0]:
        size, usage = webgpu.wgpuBufferGetSize(raw_bufs[0]), webgpu.wgpuBufferGetUsage(raw_bufs[0])
        raw_bufs[0] = utils.create_buffer(self.dev, size, usage)
        buf_patch = True
    ...
    binding_layouts += [{"binding": i+1, "visibility": webgpu.WGPUShaderStage_Compute,
                        "buffer": {"type": webgpu.WGPUBufferBindingType_Uniform if i >= len(raw_bufs) else webgpu.WGPUBufferBindingType_Storage }} for i in range(len(raw_bufs)+len(vals))]  # noqa: E501
    bindings = [{"binding": 0, "resource": {"buffer": create_uniform(self.dev, float('inf')), "offset": 0, "size": 4}}]
    bindings += [{"binding": i+1, "resource": {"buffer": create_uniform(self.dev, x) if i >= len(raw_bufs) else x, "offset": 0,
                                            "size": 4 if i >= len(raw_bufs) else webgpu.wgpuBufferGetSize(x)}} for i,x in enumerate(tuple(raw_bufs)+vals)]  # noqa: E501
    ...
    utils.set_bind_group(compute_pass ,bind_group) # last 2 not used
    utils.dispatch_workgroups(compute_pass, *global_size)  # x y z
    ...
    utils.sync(self.dev)
    if buf_patch:
      ce = utils.create_command_encoder(self.dev)
      utils.copy_buffer_to_buffer(ce, raw_bufs[0], 0, bufs[0], 0, webgpu.wgpuBufferGetSize(bufs[0]))
      cb = utils.command_encoder_finish(ce)
      utils.submit(self.dev, [cb])
    
    if wait:
      timestamps = utils.read_buffer(self.dev, query_buf).cast("Q").tolist()
      time = (timestamps[1] - timestamps[0]) / 1e9 if wait else None
...
class WebGpuAllocator(Allocator):
  ...
  def _alloc(self, size: int, options):
    ...
    return utils.create_buffer(self.dev, round_up(size, 4), webgpu.WGPUBufferUsage_Storage | webgpu.WGPUBufferUsage_CopyDst | webgpu.WGPUBufferUsage_CopySrc)
